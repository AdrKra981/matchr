...
from app.repository.agent_messages_repository import get_messages
...
@router.get("/history")
def agent_history(current_user: User = Depends(get_current_user)):
    return get_messages(user_id=current_user.id, limit=20)